...
local streetName, playerGender, kilitsayi
...
TryToCrack = function(safeEntity)
	...
	ESX.TriggerServerCallback("safebreach:checkIfSafeIsBreachable", function(state)
		if state then
			...
			exports['mythic_notify']:DoCustomHudText('error', 'Kasayı açamazsın.', 5000)
		end
...
StartCrackingSafe = function(safeEntity)
	...
	kilitsayi = (360 - currentSafeCorrectPosition) / 36
	kilitsayi = math.floor(kilitsayi)
	local kilitsayirandom = kilitsayi + 2
	local random1 = kilitsayi - math.random(1, 2)

	if random1 == kilitsayirandom then
        kilitsayirandom = kilitsayi + 2
		random1 = kilitsayi - math.random(1, 2)
	end
	
	if random1 > 10 or random1 == 10 or kilitsayirandom > 10 or kilitsayirandom == 10 or random1 < 0 or kilitsayirandom < 0  then
		exports['mythic_notify']:DoCustomHudText('error', 'Şifreyi bulamadın!', 5000)
	else
		if random1 > kilitsayirandom and random1 ~= kilitsayirandom then
			exports['mythic_notify']:DoCustomHudText('inform', 'Şifre şu aralıkta ' .. kilitsayirandom .. '0, ' .. random1 .. '0', 5000)
			else
			exports['mythic_notify']:DoCustomHudText('inform', 'Şifre şu aralıkta ' .. random1 .. '0, ' .. kilitsayirandom .. '0', 5000)
			end
	end


	if random1 == kilitsayirandom then
	exports['mythic_notify']:DoCustomHudText('inform', 'Şifre şu aralıkta ' .. random1 .. '0, ' .. kilitsayirandom + 2 .. '0', 5000)
	end
			...
			if IsControlJustPressed(0, 22) then
				if dialRotation == currentSafeCorrectPosition then
					...
					if #safeLocks == 1 then
						...
						TriggerServerEvent("esx_phone:send", "police", "GPS Market soygunu ihbarı! konumu haritada işaretlemek için buraya basın", true, { ["x"] = x, ["y"] = y, ["z"] = z })
						TriggerServerEvent('esx_outlawalert:robberyInProgress', {
							x = ESX.Math.Round(x, 1),
							y = ESX.Math.Round(y, 1),
							z = ESX.Math.Round(z, 1)
						}, streetName, playerGender, 'Market ')
					end
					...
					if #safeLocks < Config.TotalLocks then
						...
						ESX.TriggerServerCallback("safebreach:safeBreached", function(breached)
							if breached then
								...
								exports['mythic_notify']:DoCustomHudText('inform', 'Kasayı açtın!', 5000)
							else
								exports['mythic_notify']:DoCustomHudText('error', 'Kasa zaten açık!', 5000)
							end
						end, {
							["store"] = currentStreetName,
							["doorCoords"] = {
					...
					exports['mythic_notify']:DoCustomHudText('error', 'Yanlış şifre!', 5000)
				end
...
TakeSafeItem = function(itemEntity, safeData)
	...
	if DoesEntityExist(itemEntity) then
		...
		ESX.TriggerServerCallback("safebreach:receiveReward", function(received)
			if received then
				exports['mythic_notify']:DoCustomHudText('error', 'Şu eşyayı çaldın' .. stoleItem, 5000)
			end
		end, stoleItem)

		DeleteEntity(itemEntity)
	else
		exports['mythic_notify']:DoCustomHudText('error', 'Kasayı senden önce biri açmış!', 5000)
	end
end
