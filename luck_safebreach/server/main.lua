	...
	if player then
		if reward then
			if reward == "money" then
				...
				exports['mythic_notify']:DoCustomHudText('inform', 'Şu kadar para çaldın!' .. rewardAmount, 5000)
			else
				player.addInventoryItem("champagne", 1)
				
				exports['mythic_notify']:DoCustomHudText('inform', 'Yıllanmış şarap çaldın!', 5000)
			end
